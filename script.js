...
Book.prototype.info= function() {
 ...
 Book.prototype.status= function(bookId, value){
  myLibrary[bookId].read = value;
}
...
function addBookToLibrary(){
  ...
  let book = [];
    form.forEach(input => {
      ...
      input.type != 'checkbox' ? book.push(input.value) : book.push(Boolean(input.checked));
...
//name of this function is not the best
...
function cardGen(book){
 ...
 list.forEach(style=>{
  ...
  img.setAttribute("onclick", "deleteBook(this.parentNode.getAttribute('data-index')); this.parentNode.remove();")
  // css
 ...
 Object.keys(book).forEach(key=>{
  ...
  if(book.title==book[key]){
    ...
    input.type='checkbox';
    input.name= 'status';
    input.setAttribute('onchange', "myLibrary[this.parentNode.getAttribute('data-index')].status(this.parentNode.getAttribute('data-index'), this.checked);")
